#In this script we want to calculate metrics of land cover from the classified composites developed in Google Earth Engine
...
#PLOT----
#Plot the relative land cover at catchment scale during the most recent period (2019-2023)
#load the file
LU_prop_outlet <- read.csv(file = "../RIMSEC_general_classification/metrics/outlet/LU_prop.csv")



LU_prop_outlet %>% filter( Period == "2019_2023",
                           Scale == "catchment") %>%
                   complete(Park, land_use, fill = list(LU_prop = 0))%>%
  ggplot(aes(x= Park, y = LU_prop*100, fill = land_use))+
  ...
  scale_fill_manual(values = c("yellow", "grey", "lightgreen", "darkgreen","brown", "purple", "red", "blue"))+
  scale_x_discrete(labels = c("Aizkorri", "Aralar", "Artikutza", "Gorbea", "Izki"))+
  labs(x = "",
       y ="Relative Land Cover")+
